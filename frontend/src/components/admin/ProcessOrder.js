...
const ProcessOrder = ({ match }) => {
    ...
    const [status, setStatus] = useState("");
      ...
      console.log('Process order_ user', user )
                    ...
                    <Fragment>
                        {loading ? <Loader /> : (
                            ...
                            <div class="col-12 col-lg-7 order-details">
                                ...
                                <h2 class="my-5">Order # {order._id}</h2>
        
                                <h4 className="mb-4">Shipping Info</h4>
                                <p><b>Name:</b> {user.name}</p>

                                {shippingInfo && shippingInfo.map(ship => (                           
                                    <div className="row">
                                    <div className="col-12" ><p><b>Phone:</b> {ship.phoneNo}</p> </div> 
                                    <div className="col-12" ><p><b>Address:</b> {ship.address} {ship.city} {ship.postalCode}</p></div>
                                    </div>

                                    ))}
                                
                                <p><b>Amount:</b> Nrs {totalPrice}</p>
        
                                <hr />
        
                                <h4 className="my-4">Payment</h4>
                        <p className={isPaid ? "greenColor" : "redColor"} ><b>
                            {isPaid ? "PAID" : "NOT PAID"}</b></p>


                            <h4 className="my-4">Stripe ID</h4>
                        <p><b>{paymentInfo && paymentInfo.id}
                            </b></p>
